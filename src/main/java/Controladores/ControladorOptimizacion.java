...
/**
 * Controlador encargado de gestionar la ejecución de algoritmos de optimización
 * y el cálculo en tiempo real de métricas globales (costos, distancias, atenuación 
 * y auditorías de estado) de la topología de red.
 */
public class ControladorOptimizacion {
    ...
    /**
     * Constructor del controlador de optimización.
     * Configura los listeners iniciales y realiza la primera auditoría de métricas.
     */
    ...
    /**
     * Configura los eventos de los botones de la interfaz relacionados con la optimización.
     */
    ...
    /**
     * Calcula métricas financieras, técnicas y operativas de toda la red.
     * Evalúa estados de orfandad, atenuación y saturación de los equipos, 
     * actualizando los colores de alerta y los paneles de la interfaz gráfica.
     */
    private void actualizarMetricasGlobales() {
        SwingUtilities.invokeLater(() -> {
            // Contabilizar nodos excluyendo a los clientes (para costos de hardware)
            ...
            // CÁLCULO DE COSTOS Y DISTANCIA
            // 1. Costo de la fibra óptica instalada (suma de enlaces activos)
            double costoCables = modelo.getEnlacesActivos().stream().mapToDouble(Enlace::getCosto).sum();
            
            // 2. Costo del hardware (solo equipos de infraestructura, excluye clientes)
            ...
            // Limpiamos todas las alertas visuales antes de iniciar la nueva auditoría
            modelo.getNodos().forEach(Nodo::limpiarAlerta);
            modelo.getEnlaces().forEach(Enlace::limpiarAlerta);

            Nodo olt = modelo.getNodos().stream()
                    .filter(n -> n.getTipo() == Modelo.dominio.TipoNodo.CENTRAL_OLT)
                    .findFirst()
                    .orElse(null);
            ...
            for (Nodo n : modelo.getNodos()) {
                ...
                // Solo es huérfano si Prim lo vio en el grafo inicial, pero no pudo conectarlo
            ...
            for (Nodo n : modelo.getNodos()) {
                ...
                if (!tieneCableActivo) {
                    if (!fueEvaluado) {
                        // Recién agregado al mapa -> GRIS (Neutral)
                        n.setColorAlerta(new Color(100, 100, 100)); 
                    } else {
                        // Prim intentó conectarlo pero no habían puertos suficientes -> AMARILLO (Huérfano)
            ...
            for (Nodo n : modelo.getNodos()) {
                ...
                // Si el equipo de distribución está lleno al 100%
                if (conexionesActivas >= limiteConexiones) {
                    
                    boolean hayDemandaInsatisfecha = false;
                    
                    // Verificamos si la saturación de este equipo está dejando nodos inactivos por fuera
            ...
            // DETERMINACIÓN DEL ESTADO LÓGICO GLOBAL
    ...
    /**
     * Ejecuta el algoritmo de enrutamiento seleccionado (ej. Prim) en un hilo secundario
     * para no bloquear la interfaz gráfica.
     */
    private void ejecutarAlgoritmo(IMSTStrategy estrategia) {
        if (modelo.getNodos().size() < 2) {
            Util.Tecnicas.GestorAlertas.mostrarAdvertencia(vista, "Datos Insuficientes", "Añada al menos 2 nodos al mapa para poder enrutar la red.");
            return;
        }
        ...
        new Thread(() -> {
            try {
                estrategia.ejecutar(modelo); // Ejecuta el algoritmo (ej. Prim)
                ...
                SwingUtilities.invokeLater(() -> {
                    Util.Tecnicas.GestorAlertas.mostrarError(vista, "Error de Enrutamiento", ex.getMessage());
    ...
    /**
     * Limpia todas las conexiones generadas y resetea las métricas de la red,
     * manteniendo intactos los nodos colocados en el mapa.
     */
    private void limpiarConexiones() {
        ...
        // Resetear ambos paneles de la interfaz
    ...
    /**
     * Genera una malla completa (grafo completo) con todos los enlaces posibles permitidos
     * para que posteriormente el algoritmo de enrutamiento elija los más óptimos.
     */
    ...
    /**
     * Dicta las leyes inmutables de cómo se puede conectar una red FTTH.
     * Valida si dos tipos de nodos pueden enlazarse físicamente.
     */
